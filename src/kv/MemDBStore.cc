...
void MSStore::_encode(btree::btree_map<string, bufferptr>:: iterator iter, bufferlist &bl)
{
...
{
  int mode = 0644;
  int fd = ::open(_get_data_fn().c_str(), O_WRONLY|O_CREAT|O_TRUNC, mode);
  if (fd < 0) {
    int err = errno;
...
{
  dtrace << __func__ << " " << prefix << " " << k << dendl;
  ops.push_back(make_pair(DELETE, std::make_pair(std::make_pair(prefix, k),
    NULL)));
}
...
int MSStore::_merge(ms_op_t &op)
{
  ...
  if (_get(op.first.first, op.first.second, &bl_old) == false) {
    std::string new_val = std::string(bl.c_str());
    /*
     * Merge non existent.
     */
    mop->merge_nonexistant(bl.c_str(), bl.length(), &new_val);
...
bool MSStore::_get(const string &prefix, const string &k, bufferlist *out)
{
  ...
  if (iter == m_btree.end()) {
    derr << "MSReadObject failed: prefix : " << prefix << " key: " << k << ":" << dendl;
    return false;
  }
...
int MSStore::MSWholeSpaceIteratorImpl::tokenize(const std::string *s,
                             char space, std::vector<std::string> *tokens)
{
  ...
  while (i1 != std::string::npos) {
    i1 = s->find(space, i2);
    tokens->push_back(s->substr(i2, i1));
    i2 = i1 + 1;
    count++;
  }
    ...
    string *prefix, string *key)
{
  ...
  count = tokenize(pkey, delim, &tokens);
