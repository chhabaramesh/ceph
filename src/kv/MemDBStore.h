...
#ifndef MEMDB_SCALE_DB_H
#define MEMDB_SCALE_DB_H
...
public:
  MSStore(CephContext *c, const string &path, void *p) :
  ...
  class MSTransactionImpl : public KeyValueDB::TransactionImpl {
    public:
    ...
    MSTransactionImpl(MSStore* _db) :m_db(_db)
    {
      ops.clear();
    }
    ~MSTransactionImpl() {};
...
private:
   ...
   * Transaction states.
   */
  typedef std::map<std::string, bufferptr> MSMultiMap;
  MSMultiMap write_ops;
  MSMultiMap merges;
...
public:
  ...
  KeyValueDB::Transaction get_transaction() {
    return std::shared_ptr<MSTransactionImpl>(new MSTransactionImpl(this));
  }
  ...
  class MSWholeSpaceIteratorImpl : public KeyValueDB::WholeSpaceIteratorImpl {
      ...
      int split_key(const string *pkey, string *prefix, string *key);
      int tokenize(const std::string *s, char space, std::vector<std::string> *tokens);

  public:
    MSWholeSpaceIteratorImpl(btree::btree_map<std::string, bufferptr> *btree_p,
...
protected:
  ...
  WholeSpaceIterator _get_iterator() {
    return std::shared_ptr<KeyValueDB::WholeSpaceIteratorImpl>(
      new MSWholeSpaceIteratorImpl(&m_btree, &m_lock));
  }
